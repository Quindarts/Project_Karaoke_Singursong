		...
		table_dvDatPhong.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(java.awt.event.MouseEvent evt) {
				...
				if (col == 2) {
					...
					JDialog_CapNhatSoLuong md = new JDialog_CapNhatSoLuong(table_dvDatPhong, tableDichVu, selectedItem,
							col, row);
					md.setVisible(true);
				}
			}
		...
		if (o.equals(btn_xacNhan)) {
			...
			for (int i = 0; i < table_dvDatPhong.getRowCount(); i++) {
				...
				try {
					...
					ThongTinDichVu ttdv = dv.getThongTinDichVu();

					ttdv.setSoLuongDaSuDung(dv.getThongTinDichVu().getSoLuongDaSuDung() + ctdv.getSoLuong());
					DAO_TTDV.capNhatThongTinDichVu(ttdv);
					System.out.println(ttdv.getSoLuongDaSuDung());
				} catch (Exception e1) {
					e1.printStackTrace();
				}
					...
					dsCTDV_HD.forEach(ctdv -> {
						...
						for (int i = 0; i < tableDichVu.getRowCount(); i++) {
							...
							if (dv.getMaDichVu().equals(tableDichVu.getValueAt(i, 0).toString())) {
								...
								tableDichVu.setValueAt(String.valueOf(dv.getThongTinDichVu().tinhSoLuongConLai()), i,
										2);
							}
						}
						model_dichVuDatPhong.addRow(rowData);
